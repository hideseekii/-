...
for epoch in range(n_epochs):
    ...
    print(f"Epoch {epoch+1}/{n_epochs}: training accuracy: {training_accuracy}, valid accuracy: {valid_accuracy}, valid loss: {valid_loss} , time: {epoch_time:.2f} seconds")
...
test= ImageFolder("project_b/val20", transform=test_transform, target_transform=None)
...
with torch.no_grad():
    for batch, _ in test_loader:
        ...
        batch_labels = torch.argmax(batch_outputs, dim=1).cpu().numpy()
        predicted_labels.append(batch_labels)

predicted_labels = np.concatenate(predicted_labels)

df = pd.read_csv('projectB_ans.csv')
df = df.iloc[:1000]
result_df = pd.DataFrame({
    "label": predicted_labels
})
df["label"] =  predicted_labels
df.to_csv('projectB_ans.csv', index=False)
