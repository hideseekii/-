...
#read
train = ImageFolder("project_b/B_traing1", transform=train_transform, target_transform=None)
new = ImageFolder("../../TOPIC/ProjectB/B_traing2", transform=train_transform, target_transform=None)
train = ConcatDataset([train,new])
...
for epoch in range(n_epochs):
    ...
    print(f"[Epoch {epoch+1}/{n_epochs}] trainacc:{training_accuracy:.3f} | trainloss:{training_loss:.4f} | valacc:{valid_accuracy:.3f} | valloss:{valid_loss:.4f} | lr:{c_lr:.5f} | time:{epoch_time:.0f}s")
...
torch.save(model, 'model/B_model.pt')